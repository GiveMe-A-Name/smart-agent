...
- **Detailed design belongs in the approval surface.** When a plan introduces or changes abstractions, flows, contracts, or key logic, include an RFC-style Detailed Design in the Human Review Section: architecture diagram, abstraction model, data/contract shape, and pseudocode for approval-critical logic [because humans need to approve the solution design before an agent turns it into repository edits]
...
All plan sizes need a Human Review Section first. When the Detailed Design Gate is triggered, the design is a `### Detailed Design` subsection inside the Human Review Section, immediately after the human summary/decision fields and before technical assessment. See `references/human-readable-summary.md` when writing or revising that section's fields, freeze rules, or examples.
...
- Human Review Section with Intent, Layer 1, Layer 2 Approach and Design Rationale, Change Snapshot when triggered, Task Overview, Key Decisions or `None requiring human ratification`, conflict priority when goals can compete, and Detailed Design when triggered
- Detailed Design appears inside the Human Review Section before any technical assessment, risks, stop signals, file map, or task checklist [because it explains the approved solution design to the plan reader before agent-facing evidence and execution controls begin]
...
Include a `### Detailed Design` subsection inside the Human Review Section when the chosen solution design is approval-relevant, meaning a plan reader could approve the outcome while misunderstanding what abstractions, flows, contracts, or key logic the agent is about to create. Observable triggers:
...
- the work introduces or changes approval-critical algorithms, branching logic, reconciliation behavior, retry behavior, fallback behavior, conflict resolution, scheduling, parsing, validation, authorization, migration, or state transitions
...
Omit the section when none of the triggers above are present. The number of edited files is not a trigger by itself: helper modules, companion tests, fixtures, docs, generated files, localized behavior changes, or same-layer control-flow edits inside an established boundary do not require Detailed Design unless they also change an architecture boundary, ownership boundary, cross-layer handoff, contract shape, or approval-critical logic.

The Detailed Design is a reader-facing explanation of the chosen technical solution, not an execution checklist or file-level implementation map. Write it so a plan reader can understand what the solution is doing, why that shape fits the design intent, what abstractions and flows will exist, and how the approval-critical logic behaves. Because the section is part of the Human Review Section, use stable architecture anchors such as package names, module areas, layers, entrypoint roles, public seams, dependency direction, ownership boundaries, public/internal contract names, and domain data shapes. Do not use line numbers, exact edit lists, task steps, or private local details. Include:
...
- **Abstraction design:** the new or changed abstractions by name or role, the responsibility each owns, why each abstraction exists, and which existing abstraction it replaces, wraps, extends, or deliberately avoids. If no new abstraction is introduced, write `No new abstraction; existing [module/component/service] owns the behavior`.
- **Architecture diagram:** a Mermaid diagram or compact ASCII diagram showing the abstractions/modules, call/data flow, dependency direction, and ownership boundaries. Required whenever the design includes two or more abstractions or a new cross-boundary abstraction; optional only when the design names exactly one changed existing module with no new dependency edge.
...
- **Contract/data shape:** the important request, response, event, schema, state, configuration, or error shapes needed to understand the design. Use compact field lists or shape examples only when they are approval-relevant; omit private helper signatures.
- **Key logic pseudocode:** pseudocode for approval-critical algorithms or branching behavior. Include it when behavior depends on nontrivial ordering, fallback, retry, conflict resolution, validation, migration, state transition, or orchestration. Omit it for simple straight-line plumbing.
...
Do not include full production code, incidental variable names, line-level edit instructions, exhaustive private APIs, speculative future abstractions, file maps, or task checklists. Stable package names, public/internal contract names, directory-level architecture anchors, compact type/data shapes, and pseudocode are allowed when they are needed for the human to understand the design. Exact files-to-edit belong in File Map. Do not invent generic abstraction layers such as "manager", "service", or "handler" without naming the concrete responsibility and caller relationship [because vague abstraction labels create the impression of design while hiding where behavior will actually live]. If a required abstraction boundary, contract shape, or key logic choice is unknown, surface it in Key Decisions, add an uncertainty-reduction task, or add a stop signal instead of inventing a design.

If unsure about the expected design granularity, read `references/detailed-design-examples.md`.
...
- If a Detailed Design exists, every architecture-touching or key-logic-changing task must either preserve the approved abstraction model, flow, contract/data shape, and pseudocode semantics or record a revision trigger/stop signal before choosing a different shape.
...
- When triggered, Detailed Design appears inside the Human Review Section after the human summary/decision fields and before `## Situation Assessment` or any execution-facing section.
...
- Outside `### Detailed Design`, Human Review Section text contains no exact files-to-edit, line numbers, implementation-only function/method/class/module names, internal APIs, or non-user-visible CLI flags. Stable package names, directory-level architecture anchors, public/developer-facing endpoints, fields, schema names, config keys, event names, CLI options, public SDK/plugin methods, and contract names are allowed only when they are the behavior or target architecture being approved. Inside `### Detailed Design`, stable internal contract names, compact contract/data shapes, and pseudocode are allowed only when they are needed to show the approved solution shape; exact files-to-edit, line-level edits, task steps, private helper details, and exhaustive private APIs still belong outside the Human Review Section.
...
Read `references/detailed-design-examples.md` when a plan triggers the Detailed Design Gate and the right solution-design granularity is unclear. The reference contains backend, frontend, CLI/pipeline, and anti-pattern examples.
