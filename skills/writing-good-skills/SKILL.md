...
- [ ] If the change addresses behavior induced by existing skill text, the inducing source was named before editing, and the diff removes or neutralizes that source before adding any negative boundary.
...
- a new counter-rule was added for behavior made salient by existing wording, examples, or workflow, while the inducing source remains in place
